...
public class DefaultPreviewStore implements PreviewStore {
  ...
  private static final DatasetId PREVIEW_PENDING_TABLE_ID = NamespaceId.SYSTEM.dataset("preview.pending.table");
  ...
  private final LevelDBTableCore dataAndMdTable;
  private final LevelDBTableCore pendingTable;
  ...
  DefaultPreviewStore(@Named(PreviewConfigModule.PREVIEW_LEVEL_DB) LevelDBTableService service) {
    try {
      ...
      this.dataAndMdTable = new LevelDBTableCore(PREVIEW_TABLE_ID.getDataset(), service);
      service.ensureTableExists(PREVIEW_PENDING_TABLE_ID.getDataset());
      this.pendingTable = new LevelDBTableCore(PREVIEW_PENDING_TABLE_ID.getDataset(), service);
    } catch (IOException e) {
  ...
  public void put(ApplicationId applicationId, String tracerName, String propertyName, Object value) {
    ...
    try {
      dataAndMdTable.put(mdsKey.getKey(), TRACER, Bytes.toBytes(tracerName), 1L);
      dataAndMdTable.put(mdsKey.getKey(), PROPERTY, Bytes.toBytes(propertyName), 1L);
      dataAndMdTable.put(mdsKey.getKey(), VALUE, Bytes.toBytes(gson.toJson(value)), 1L);
    } catch (IOException e) {
  ...
  public Map<String, List<JsonElement>> get(ApplicationId applicationId, String tracerName) {
    ...
    try (Scanner scanner = dataAndMdTable.scan(startRowKey, stopRowKey, null, null, null)) {
      Row indexRow;
  ...
  private void removePreviewData(byte[] prefix, ApplicationId applicationId) {
    ...
    byte[] stopRowKey = new MDSKey(Bytes.stopKeyForPrefix(startRowKey)).getKey();
    try {
      dataAndMdTable.deleteRange(startRowKey, stopRowKey, null, null);
    } catch (IOException e) {
  ...
  public void setProgramId(ProgramRunId programRunId) {
    ...
    try {
      dataAndMdTable.put(mdsKey.getKey(), RUN, Bytes.toBytes(gson.toJson(programRunId)), 1L);
    } catch (IOException e) {
  ...
  public ProgramRunId getProgramRunId(ApplicationId applicationId) {
    ...
    try {
      row = dataAndMdTable.getRow(mdsKey.getKey(), new byte[][]{RUN},
                                  null, null, -1, null);
    } catch (IOException e) {
  ...
  public void setPreviewStatus(ApplicationId applicationId, PreviewStatus previewStatus) {
    ...
    MDSKey mdsKey = getPreviewRowKeyBuilder(META_ROW_KEY_PREFIX, applicationId).build();
    try {
      dataAndMdTable.put(mdsKey.getKey(), STATUS, Bytes.toBytes(gson.toJson(previewStatus)), 1L);
      dataAndMdTable.put(mdsKey.getKey(), APPID, Bytes.toBytes(gson.toJson(applicationId)), 1L);
    } catch (IOException e) {
  ...
  public PreviewStatus getPreviewStatus(ApplicationId applicationId) {
    ...
    Map<byte[], byte[]> row = null;
    try {
      row = dataAndMdTable.getRow(mdsKey.getKey(), new byte[][]{STATUS},
                                  null, null, -1, null);
    } catch (IOException e) {
  ...
  public void add(ApplicationId applicationId, AppRequest appRequest) {
    ...
    try {
      pendingTable.put(mdsKey.getKey(), APPID, Bytes.toBytes(gson.toJson(applicationId)), 1L);
      pendingTable.put(mdsKey.getKey(), CONFIG, Bytes.toBytes(gson.toJson(appRequest)), 1L);
  ...
  private void removeFromWaitingState(ApplicationId applicationId) {
    ...
    try {
      pendingTable.deleteRows(Collections.singleton(mdsKey.getKey()));
    } catch (IOException e) {
  ...
  public List<PreviewRequest> getAllInWaitingState() {
    ...
    try (Scanner scanner = pendingTable.scan(startRowKey, stopRowKey, null, null, null)) {
      Row indexRow;
      while ((indexRow = scanner.next()) != null) {
        Map<byte[], byte[]> columns = indexRow.getColumns();
  ...
  private void setPollerinfo(ApplicationId applicationId, byte[] pollerInfo) {
    ...
    try {
      dataAndMdTable.put(mdsKey.getKey(), POLLERINFO, pollerInfo, 1L);
    } catch (IOException e) {
  ...
  public byte[] getPreviewRequestPollerInfo(ApplicationId applicationId) {
    ...
    Map<byte[], byte[]> row = null;
    try {
      row = dataAndMdTable.getRow(mdsKey.getKey(), new byte[][]{POLLERINFO},
                                  null, null, -1, null);
    } catch (IOException e) {
  ...
  void clear() throws IOException {
    ...
    service.ensureTableExists(PREVIEW_TABLE_ID.getDataset());
    service.dropTable(PREVIEW_PENDING_TABLE_ID.getDataset());
    service.ensureTableExists(PREVIEW_PENDING_TABLE_ID.getDataset());
  }
}
